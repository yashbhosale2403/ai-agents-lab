...
> **What's in this folder:** the exported Bedrock Flow definition (`flows/cybersecurity-quiz-flow.json`, AWS account ID redacted) and this design write-up. The Lambda, AstraDB loader and test files described under *Project Structure* are part of the design and are not included here.
...
### Bedrock Flow in this folder

Generated from the node and connection list in [`flows/cybersecurity-quiz-flow.json`](flows/cybersecurity-quiz-flow.json):

```mermaid
flowchart LR
    IN["FlowInputNode<br/>(Input · document: String)"] -->|"$.data"| AG["AgentsNode_1<br/>(Bedrock Agent · security expert)"]
    AG -->|"$.data"| OUT["FlowOutputNode<br/>(Output · quiz JSON)"]
```
